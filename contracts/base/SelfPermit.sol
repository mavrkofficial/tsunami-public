...
abstract contract SelfPermit is ISelfPermit {
    /// @dev ERC-2771 compatible sender — overridden by NPM to extract real sender from relay calldata.
    ///      Default returns msg.sender for non-relayed calls.
    ...
    function selfPermitAllowedIfNecessary(
        ...
        if (IERC20(token).allowance(_msgSender(), address(this)) < type(uint256).max)
